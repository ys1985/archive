...
s.linkInternalFilters="javascript:,gmobb.jp"
...
function s_doPlugins(s) {
	...
	s.campaign=s.getQueryParam('banner_id');
	s.eVar1=s.getQueryParam('banner_id');
	...
	if(s.campaign){
		...
		s.prop22=s.getValOnce(decodeURIComponent(escape(s.getQueryParam('OVKEY,GGKEY'))),'gmoingmobb_prop22',0); // Listing-Ad Keyword
		s.eVar22=s.prop22;

		s.prop23=s.getValOnce(decodeURIComponent(escape(s.getQueryParam('OVADID,GGADID'))),'gmoingmobb_prop23',0); // Listing-Ad Creative
		s.eVar23=s.prop23;
	}

	/* AD or notAD */
	if(s.campaign){s.setCookie(s.campaign,"gmoingmobb_prop24",0);}
	if(s.getCookie("gmoingmobb_prop24") == false){s.prop24='notAD';}
	...
	s.eVar26=s.getValOnce(s.prop26,'gmoingmobb_eVar26', 0);

	/* Time */
	s.prop27=s.getTimeParting('h','9',new Date().getFullYear()); // Time at intervals of 15 minutes
	s.eVar27=s.getValOnce(s.prop27,'gmoingmobb_eVar27', 0);

	/* Landing Page */
	s.prop28=s.getValJustOnce(s.pageName,'gmoingmobb_prop28', 0);
	s.eVar28=s.prop28;

	/* Pass of Campaign */
	if(s.getCookie("gmoingmobb_prop24")){s.prop29="|"+s.getCookie("gmoingmobb_prop24")+"|"+s.pageName;}
	if(s.getCookie("gmoingmobb_prop24") == false){s.prop29="|notAd|"+s.pageName;}

	/* Pass of 1st or Repeat visit */
	if(s.prop25 == 'New'){s.prop30= "|New|"+s.pageName;}
	if(s.prop25 == 'Repeat'){s.prop30= "|Repeat|"+s.pageName;}

	/* Campaign click history */
	s.eVar29=s.crossVisitParticipation(s.campaign,'gmoingmobb_xvp_c','45','5','>','');

	/* Listing Keyword click history */
	s.eVar30=s.crossVisitParticipation(s.prop23,'gmoingmobb_xvp_k','45','5','>','');

	/* other campaigns */
	s.prop41=s.getQueryParam('intcid'); //internal campaign
	s.eVar41=s.prop41;

	s.prop42=s.getQueryParam('sid');
	s.eVar42=s.prop42;

	s.prop43=s.getQueryParam('pid');
	s.eVar43=s.prop43;

	s.prop44=s.getQueryParam('lcid');
	s.eVar44=s.prop44;

	/* event tracking */
//	if(s.events){s.prop45="["+s.events+"]"+s.prop50;}
	if(s.events){if(s.pageName){s.prop45="["+s.events+"]"+s.pageName;}
			else {s.prop45="["+s.events+"]"+s.prop50;}}

	/* document.title */
	s.prop47 = document.title;
...
/* for bounce rate */
if(document.URL.match(/gmobb/i)){
	if(document.referrer.match(/gmobb/i)){
		if(s.events){
			s.events=s.events+','+s.getValOnce("event20",'gmobb_events',0);
		}else{
			s.events=s.getValOnce("event20",'gmobb_events',0);
		}
	}else{
		if(s.events){
			s.events=s.events+','+s.getValJustOnce("event19",'gmobb_events',0);
		}else{
			s.events=s.getValJustOnce("event19",'gmobb_events',0);
		}
	}
}
	...
	/* pid Tracking */
	s.eVar12=s.getQueryParam('pid');
	s.prop12 = s.eVar12;
	if(s.eVar12){s.setCookie(s.eVar12,"gmoingmobb_evar12",0);}
	s.prop15 = s.getCookie("gmoingmobb_evar12");
	s.prop18="["+s.getCookie("gmoingmobb_evar12")+"]"+s.prop50;
	if(s.pageName){s.prop18="["+s.getCookie("gmoingmobb_evar12")+"]"+s.pageName;}
	
	/* sid Tracking */
	s.eVar13 = document.URL.split("#")[1];	//hash Tracking
	if(!s.eVar13){s.eVar13=s.getQueryParam('sid');}
	s.prop13 = s.eVar13;
	if(s.eVar13){s.setCookie(s.eVar13,"gmoingmobb_evar13",0);}
	s.prop16 = s.getCookie("gmoingmobb_evar13");
	s.prop19="["+s.getCookie("gmoingmobb_evar13")+"]"+s.prop50;
	if(s.pageName){s.prop19="["+s.getCookie("gmoingmobb_evar13")+"]"+s.pageName;}

	/* banner_id Tracking */
	s.eVar14=s.getQueryParam('banner_id');
	s.prop14 = s.eVar14;
	if(s.eVar14){s.setCookie(s.eVar14,"gmoingmobb_evar14",0);}
	s.prop17 = s.getCookie("gmoingmobb_evar14");
	s.prop20="["+s.getCookie("gmoingmobb_evar14")+"]"+s.prop50;
	if(s.pageName){s.prop20="["+s.getCookie("gmoingmobb_evar14")+"]"+s.pageName;}
	
	
	/* pid+sid Tracking */
	s.prop11 = s.prop15+"-"+s.prop16;
	s.eVar15 = s.prop11;

	
	/* sid on purchase Tracking */
	if(s.events){if(s.events.match(/purchase/i)){s.eVar16 = s.prop16;};}

	if(s.events){if(s.events.match(/purchase/i) && s.prop16.match(/flo|flets/i)){s.events=s.events+',event41';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16.match(/eao|eacc/i)){s.events=s.events+',event42';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16.match(/mbo|emob/i)){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16.match(/ipo|static/i)){s.events=s.events+',event44';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16.match(/wmo|wmob/i)){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16.match(/lto|lte/i)){s.events=s.events+',event46';};}
	
	/*sid ’Ç‰Á*/
	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'mb2'){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'c_gp02'){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'mb_ssn'){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gp02'){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gp02'){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'wa36'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'wmc'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_wa36'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_wa36'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_wmc'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_wmc'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'mb_ssn2'){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'wu10'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_wu10'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_wu10'){s.events=s.events+',event45';};}		/*sid’Ç‰Á1208*/	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kpn04'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_wa36o'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_wa36c'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl01p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl01p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl01p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl01p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl01p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl01p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl02p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl02p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl02p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl02p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl02p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl02p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl03d2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl03d2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl03d2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl03d1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl03d1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl03d1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl04p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl04p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl04p2'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'gl04p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kkk_gl04p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'hik_gl04p1'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kpn05'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kpn06'){s.events=s.events+',event45';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kpn07'){s.events=s.events+',event43';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kpn08'){s.events=s.events+',event46';};}	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'kpn09'){s.events=s.events+',event46';};}
	
	/*sid’Ç‰Á1209*/
	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'nex1'){s.events=s.events+',event47';};}
	if(s.events){if(s.events.match(/purchase/i) && s.prop16 == 'nex2'){s.events=s.events+',event47';};}

	/* campaign pages Tracking */
	if(s.server.match(/muscle/i)){s.eVar7 = s.server;}	if(s.server.match(/3rd_anniversary/i)){s.eVar7 = s.server;}
	
	/* campaign pages Tracking */	
	s.prop31=s.getPreviousValue(s.prop50,"s_pv_c50");
	if (s.prop31){
