...
//	if(s.events){s.prop45="["+s.events+"]"+s.prop50;}
	if(s.events){s.prop45="["+s.events+"]"+s.pageName;}
	...
	/* for bounce rate */
	if(document.URL.match(/gmobb.jp/i)){
		if(document.referrer.match(/gmobb.jp/i)){
			if(s.events){
				s.events=s.events+','+s.getValOnce("event20",'gmoingmobb_events',0);
			}else{
				s.events=s.getValOnce("event20",'gmoingmobb_events',0);
			}
		}else{
			if(s.events){
				s.events=s.events+','+s.getValJustOnce("event19",'gmoingmobb_events',0);
			}else{
				s.events=s.getValJustOnce("event19",'gmoingmobb_events',0);
			}
		}
	}
	...
	if(s.events.match(/15/))	{if(s.getCookie("gmoingmobb_evar2").match(/kpn/)){s.events='event16';}}
	if(s.events.match(/25/))	{if(s.getCookie("gmoingmobb_evar2").match(/kkk/)){s.events='event27';}}
	if(s.events.match(/15/))	{if(s.getCookie("gmoingmobb_evar2").match(/kkk/)){s.events='event17';}}
